...
context(ctx: GeneratorContext)
public fun <A, B, C> ((A, B) -> C).from(
    first: Generator<A>,
    second: Generator<B>
): Generator<C> = Generator { random ->
    this@from(first.next(random), second.next(random))
}

context(ctx: GeneratorContext)
public fun <A, B, C, D> ((A, B, C) -> D).from(
    first: Generator<A>,
    second: Generator<B>,
    third: Generator<C>
): Generator<D> = Generator { random ->
    this@from(first.next(random), second.next(random), third.next(random))
}
